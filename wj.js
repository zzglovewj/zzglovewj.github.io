...
if(now>now1){
	document.getElementById("like").innerHTML="跨年快乐"
}
...
function djstime(){
				var now=new Date()
				var now1=Date.parse("2024/1/1")
				var time=(now1-now)/1000
				...
				var time1=`${d}天${h}时${m}分${s}秒`
				if(now<now1){
				
				document.getElementById("love").innerHTML="跨年距离目前还有"+time1
			}				
			
			else{
					clearInterval(jsq)
					document.getElementById("love").innerHTML=`${zzg}2024年新年快乐`
			}
			}
...
//三行情诗
      var num=Math.floor(Math.random()*7)
		...
		const text1 =  ['岁末将至，敬颂东缓，平安喜乐，万事胜意，年年皆胜意，岁岁长欢愉。','愿你的未来纯净明朗，想你此刻的可爱目光在时间的美好命运中，愿你的命运美好欢畅。','愿你有你灿烂的前程，愿你有情人终成眷属，愿你在尘世获得幸福。','愿你被这个世界温柔相待，愿你目之所及，心之所向慢慢都是爱，愿你绽放如花，愿你长开不败。','愿你有人陪你颠沛流离，如果没有，愿你成为自己的太阳。','万物更新，旧疾当愈，岁岁无虞，长安常安。','我对你的喜欢大抵是燃烧的星星坠落到海底开成了珊瑚，又或是揉碎的白云环绕山川流成大河，那种梦幻不真切，却令人迷醉的心动，就像晚风拥抱月亮，海风亲吻礁石。']
		
		let idx1 = 1
		writeText()
		function writeText() {
			textEl.innerText = text1[num].slice(0, idx1)
			idx1++
...
now1=new Date("2024/1/1")	
	...
	//循环三行情诗
if(now<now1){
	const textEl1 = document.getElementById('wj')
	const text2 =  ['岁末将至，敬颂东缓，平安喜乐，万事胜意，年年皆胜意，岁岁长欢愉。','愿你的未来纯净明朗，想你此刻的可爱目光在时间的美好命运中，愿你的命运美好欢畅。','愿你有你灿烂的前程，愿你有情人终成眷属，愿你在尘世获得幸福。','愿你被这个世界温柔相待，愿你目之所及，心之所向慢慢都是爱，愿你绽放如花，愿你长开不败。','愿你有人陪你颠沛流离，如果没有，愿你成为自己的太阳。','万物更新，旧疾当愈，岁岁无虞，长安常安。','我对你的喜欢大抵是燃烧的星星坠落到海底开成了珊瑚，又或是揉碎的白云环绕山川流成大河，那种梦幻不真切，却令人迷醉的心动，就像晚风拥抱月亮，海风亲吻礁石。']
	...
	function qsxh(){
		...
		writeText()
		function writeText() {
			textEl1.innerText = text2[num].slice(0, idx2)
			idx2++
		...
		}setInterval("qsxh()",10000)
		}
		
if(now>now1){  

	const textEl = document.getElementById('wj')
const text = '王菁，2024年新年快乐！当进入新的一年的那一刻，忘掉之前的所有烦恼，往后的日子都是崭新的。新的一年，愿日子如熹光温柔又安详，愿你赤诚又勇敢欣喜也在望，爱的人都喜乐如常，盼的事都归于心上，希望你在2024年每天开心。这是我第一次和你说新年快乐,有很多很多想要对你说的话，愿你做一个小太阳，而我这颗万千星辰里最普通的一粒星，会永远围绕在你的左右，我爱你，祝你新年快乐，也希望我明年也可以陪你度过这个美好的时刻。祝福说完了，嘿嘿，如果小菁看到这里，心里可以默默对我说一句新年快乐吗，接下来是我的总结：这个跨年网页祝福是我上个星期六开始准备的，一星期都没睡好觉工程量真的很大，头发掉了一大堆，黑眼圈也重了很多，但是付出的都是一切值得，因为我想让你知道你对我很重要，我喜欢你'
let idx = 1	
    writeText()
function writeText() {
...
var b=Date.parse("2023.3.30")
tian=Math.floor(((a-b)/1000/60/60/24))
			nian=Math.floor(((a-b)/1000/60/60/24/365))
			tian2=(Math.floor((a-b)/1000/60/60/24))
			i=0
				function systime(){
					tian1=tian-nian*365
					var now=new Date()
					var shi=now.getHours()
					var miao=now.getMinutes()
					var fen=now.getSeconds()
					if(tian2==0)
						tian2=1
					if(miao<10)
					miao="0"+miao
					...
					if(tian1<10)
					tian1="0"+tian1
					var timejs=`${nian}年${tian1}天${shi}时${miao}分${fen}秒`
					
					now=new Date()
					now1=new Date("2024/1/1")
					if(now<now1){ 
						document.getElementById("jk").innerHTML=`我们认识已经:${timejs}`
					...
					function f1(){
						if(i<tian2){
							i=i+1
						document.getElementById("jk").innerHTML= `想你${i}天`}
						else{
									clearInterval(interval)
								}
								}	interval=setInterval(f1,52)
								
					
				}
					}var timeid=setInterval("systime()",100)	
